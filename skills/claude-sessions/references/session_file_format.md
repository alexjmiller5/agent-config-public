...
**Path normalization**: the project's **absolute** working-directory path is encoded by replacing every `/` and `.` with `-`. It is the full absolute path, **not** the basename - a bare project name never matches.
...
- **Role lives in `message.role`**; the top-level `type` only labels the line. Older sessions stored `role`/`content` at the top level, so a robust extractor reads both locations, e.g. `role = data.get("role") or data.get("message", {}).get("role")`.
- `message.content` is either a string or an array of content blocks (`text`, `tool_use`, `tool_result`, ...).

### Non-message event lines

Recent sessions also interleave non-conversation event lines. Their `type` is one of `attachment`, `system`, `summary`, `last-prompt`, `queue-operation`, `custom-title`, `mode`, etc. These carry no `message.role` and no recoverable text/tool content - extractors should skip any line whose role is neither `user` nor `assistant`.
